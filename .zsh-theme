...
if [ $? = 0 ]; then
    hg prompt --angle-brackets "\
        ...
        %{$fg_bold[red]%}<status>%{$reset_color%}\
        %{$reset_color%}<
...
PROMPT="%{$fg_bold[$(terminal_color)]%}%n@%m%{$reset_color%} %{$fg_bold[green]%}%~%{$reset_color%}$(hg_prompt_info>/dev/null) $(git_prompt_info)%{$fg_bold[yellow]%}λ%{$reset_color%} "
