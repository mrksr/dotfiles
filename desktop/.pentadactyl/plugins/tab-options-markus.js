...
let onUnload = overlay.overlayObject(gBrowser, {
    addTab: util.wrapCallback(function addTab(uri, params, charset, postData, ownerTab) {
        ...
        if ("groupleft" == location)
            while (index > 0 && visible[index].dactylGroup && visible[index].dactylGroup == tab.dactylGroup)
                index--;
        else if ("groupright" == location)
            while (index < visible.length && visible[index].dactylGroup && visible[index].dactylGroup == tab.dactylGroup)
                index++;
